...
if (!isset($_POST["editMovieButton"]) && !isset($_POST["updateMovieButton"])) {
    header("Location: dashboard.php");
}

if (isset($_POST["editMovieButton"]) && !empty($_POST["editMovieButton"])) {

    $regexId = "/^[0-9]+$/";
    $movieId = htmlspecialchars($_POST["editMovieButton"]);
...
if (isset($_POST["updateMovieButton"])) {
    ...
    $regexId = "/^[0-9]+$/";
    $movieId = htmlspecialchars($_POST["updateMovieButton"]);
    $arrayErrors = [];
    ...
    if (!preg_match($regexTextSmall, $_POST["movieGender"])) {
        ...
        $arrayMovieUpdateInformations["movieGender"] = htmlspecialchars($_POST["movieGender"]);
    ...
    if (!preg_match($regexDuration, $_POST["movieDuration"])) {
        ...
        $arrayMovieUpdateInformations["movieDuration"] = htmlspecialchars($_POST["movieDuration"]);
    ...
    if (!preg_match($regexTextSmall, $_POST["movieNationality"])) {
        ...
        $arrayMovieUpdateInformations["movieNationality"] = htmlspecialchars($_POST["movieNationality"]);
    ...
    if (!preg_match($regexDate, $_POST["movieReleaseDate"])) {
        ...
        $arrayMovieUpdateInformations["movieReleaseDate"] = htmlspecialchars($_POST["movieReleaseDate"]);
    ...
    if (!preg_match($regexTextSmall, $_POST["movieDirector"])) {
        ...
        $arrayMovieUpdateInformations["movieDirector"] = htmlspecialchars($_POST["movieDirector"]);
    ...
    if (!preg_match($regexTextSmall, $_POST["movieWritters"])) {
        ...
        $arrayMovieUpdateInformations["movieWritters"] = htmlspecialchars($_POST["movieWritters"]);
    ...
    if (!preg_match($regexTextMedium, $_POST["movieCasting"])) {
        ...
        $arrayMovieUpdateInformations["movieCasting"] = htmlspecialchars($_POST["movieCasting"]);
    ...
    if (!preg_match($regexTextLarge, $_POST["movieSynopsis"])) {
        ...
        $arrayMovieUpdateInformations["movieSynopsis"] = htmlspecialchars($_POST["movieSynopsis"]);
    ...
    if (filter_var($_POST["movieTrailer"], FILTER_VALIDATE_URL)) {
        if (strlen($_POST["movieTrailer"]) <= 300) {
            $arrayMovieUpdateInformations["movieTrailer"] = filter_input(INPUT_POST, "movieTrailer", FILTER_SANITIZE_URL);
            $arraySuccess["movieTrailer"] = "<i class=\"fas fa-check success-darker-message\"></i>";
        } else {
            $arrayErrors["movieTrailer"] = "L'url de la bande annonce ne doit pas dépasser 300 caractères.";
        }
    } else {
        $arrayErrors["movieTrailer"] = "L'url de la bande annonce n'est pas valide.";
    }

    // Delete old picture option
    if (array_key_exists("movieRemoveOldPicture", $_POST)) {
        if ($_POST["movieRemoveOldPicture"] == "Oui") {
            if (file_exists("../uploads/affiches/" . $movieInformations["movie_picture"])) {
                ...
                $arrayErrors["movieRemoveOldPicture"] = "Ce film n'a pas d'affiche.";
            }
        }
    } else {
        $arrayErrors["movieRemoveOldPicture"] = "Veuillez faire un choix";
    }
    ...
    if ($_FILES["moviePicture"]["name"] != "") {
        if (empty($arrayErrors)) {
            ...
            if (move_uploaded_file($_FILES['moviePicture']['tmp_name'], $repertory . $fileName)) {
                $arrayMovieUpdateInformations["moviePicture"] = htmlspecialchars($fileName);
            } else {
                $arrayErrors["moviePicture"] = "Une erreur s'est produite lors de la tentative d'upload.";
            }
        }
    } else {
        $arrayMovieUpdateInformations["moviePicture"] = $movieInformations["movie_picture"];
    }

    if (empty($arrayErrors)) {

        $movie = new Movies();
        $arrayMovieUpdateInformations["movieId"] = $securedId;

        if ($movie->updateMovie($arrayMovieUpdateInformations)) {
            $_SESSION["updateMovieSuccess"] = "<i class=\"fas fa-check\"></i> Changements sauvegardés (" . $arrayMovieUpdateInformations["movieTitle"] . ")";
            header("Location: dashboard.php");
        } else {
            $arrayErrors["updateMovie"] = "Une erreur s'est produite lors de la modification du film.";
        }
    }
}
